...
string lcs(string x, string y) {
    ...
    #pragma omp parallel default(none) shared(x, y, c, m, n) private(i, j, diag) num_threads(#threads)
    #pragma omp for
    for (int diag = 2; diag < m + n; diag++) {
        #pragma omp parallel for
        for (int i = min(m, diag - 1); i < max(1, diag - 1); i++) {
            j = diag - 1;
            if (x[i - 1] == y[i - 1]) {
                table[i][j] = table[i - 1][j - 1] + 1;
            } else {
                table[i][j] = max(c[i - 1][j], c[i][j - 1]);
            }
        }
    }
